SELECT run_migration(37, $$
	...
	delete from user_relay_connection where user_id in ( select distinct u1.id from user_account u1 inner join user_account u2 ON lower(u1.email) = lower(u2.email) and u1.created < u2.created );
	delete from user_session_token where user_id in ( select distinct u1.id from user_account u1 inner join user_account u2 ON lower(u1.email) = lower(u2.email) and u1.created < u2.created );
	delete from user_settings where user_id in ( select distinct u1.id from user_account u1 inner join user_account u2 ON lower(u1.email) = lower(u2.email) and u1.created < u2.created );
