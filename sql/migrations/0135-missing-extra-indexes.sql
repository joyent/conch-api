SELECT run_migration(135, $$
    ...
    drop index if exists device_relay_connection_device_id_idx;   -- see device_relay_connection_pkey
    drop index if exists device_report_device_id_idx;             -- see device_report_device_id_created_idx
    drop index if exists user_relay_connection_user_id_idx;       -- see user_relay_connection_pkey
    drop index if exists user_workspace_role_user_id_idx;         -- see user_workspace_role_pkey
    drop index if exists validation_state_device_id_idx;          -- see validation_state_device_id_validation_plan_id_completed_idx
    drop index if exists validation_state_member_validation_state_id_idx; -- see validation_state_member_pkey
    drop index if exists workspace_rack_workspace_id_idx;         -- see workspace_rack_pkey
