...
subtest 'Sub-Workspace' => sub {
	...
	$t->post_ok("/workspace/$id/child")->status_is(401, 'No body is bad request');
...
subtest 'Workspace Rooms' => sub {
	$t->get_ok("/workspace/$id/room")->status_is(200)
		->json_has('/0', 'Has at least 1 DC room') or die;
...
subtest 'Workspace Racks' => sub {
	...
	note("Variance: /rack in v1 returns a hash keyed by datacenter room AZ instead of an array");
	$t->get_ok("/workspace/$id/rack")->status_is(200)
		->json_has('/test-region-1a/0/id', 'Has test datacenter rack') or die;
	...
	subtest 'Add rack to workspace' => sub {
		$t->post_ok("/workspace/$sub_ws/rack")->status_is(400, 'Requires request body')
			->json_like('/error', qr//);
		$t->post_ok("/workspace/$sub_ws/rack", json => {id => $rack_id} )->status_is(303)
		...
		subtest 'Cannot modify GLOBAL workspace' => sub {
			$t->post_ok("/workspace/$id/rack", json => {id => $rack_id} )->status_is(400)
...
subtest 'Single device' => sub {
	...
	subtest 'Device attributes' => sub {
		...
		$t->post_ok('/device/TEST/triton_uuid')->status_is(400, 'Request body required');
		...
		$t->post_ok('/device/TEST/asset_tag')->status_is(400, 'Request body required');
	...
	subtest 'Device settings' => sub {
		...
		$t->post_ok('/device/TEST/settings', json => { foo => 'bar' } )->status_is(200);
		...
		$t->post_ok('/device/TEST/settings/fizzle', json => { no_match => 'gibbet' })
		  ->status_is(400, 'Fail if parameter and key do not match');
		$t->post_ok('/device/TEST/settings/fizzle', json => { fizzle => 'gibbet' })
		  ->status_is(200);
...
subtest 'Assigned device' => sub {
	$t->post_ok("/workspace/$id/rack/$rack_id/layout",
		json => {
			TEST => 1
		})->status_is(200);
...
subtest 'Hardware Product' => sub {
	...
	$t->get_ok("/hardware_product")->status_is(200)
		->json_has('/0', 'Response has at least one hardware product') or die;

	my $hw_id = $t->tx->res->json->[0]->{id};

	$t->get_ok("/hardware_product/$hw_id")->status_is(200)->json_is('/id', $hw_id);
