...
sub validate_links {
	...
	if ( $links_up < 4 ) {
		$nic_state_msg =
			"$device_id: report $report_id: CRITICAL: links_up: $nic_state_log";
		mistake $nic_state_msg;
		$nic_state_status = 0;
	}
	else {
		$nic_state_msg =
			"$device_id: report $report_id: OK: links_up: $nic_state_log";
		trace $nic_state_msg;
		$nic_state_status = 1;
	}
...
sub validate_wiremap {
	...
	for my $nic (@eth_nics) {
		...
		my $nic_peer_msg =
			  "Interface "
			. $nic->iface_name
			. " Has $peer_port, Needs either of @peer_ports";
		if ( grep /\Q$peer_port/, @peer_ports ) {
			$nic_peer_log =
				"$device_id: report $report_id: OK: Correct peer: $nic_peer_msg";
			$nic_peer_status = 1;
			info $nic_peer_log;
		}
		else {
			$nic_peer_log =
"$device_id: report $report_id: CRITICAL: Wrong peer port: $nic_peer_msg";
			$nic_peer_status = 0;
	...
	if ( $num_switches == 2 ) {
		$num_switch_log =
"$device_id: report $report_id: OK: Correct number of switches: $num_switch_msg";
		$num_switch_status = 1;
		info $num_switch_log;
	}
	else {
		$num_switch_log =
"$device_id: report $report_id: CRITICAL: Wrong number of switches: $num_switch_msg";
		$num_switch_status = 0;
	...
	for my $switch_name ( keys %{$switch_peers} ) {
		...
		my $num_ports_status;
		my $num_ports_msg =
			"Has $num_ports port(s) connected to switch $switch_name, Needs 2";
		if ( $num_ports == 2 ) {
			$num_ports_log =
"$device_id: report $report_id: OK: Correct number of peer ports: $num_ports_msg";
			$num_ports_status = 1;
			info $num_ports_log;
		}
		else {
			$num_ports_log =
"$device_id: report $report_id: CRITICAL: Wrong number of peer ports: $num_ports_msg";
			$num_ports_status = 0;
...
sub switch_peer_ports {
	...
	defined $rack_index
		or error 'Device assigned to rack unit not in rack layout';
