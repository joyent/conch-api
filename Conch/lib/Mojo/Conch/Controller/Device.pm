package Mojo::Conch::Controller::Device;

use Mojo::Base 'Mojolicious::Controller', -signatures;
use Data::Validate::UUID 'is_uuid';

use aliased 'Mojo::Conch::Class::DeviceDetailed';
...
sub under ($c) {
  ...
  my $maybe_device = $c->device->lookup_for_user($c->stash('user_id'), $device_id);
...
sub get ($c) {
  ...
  if ($device_report->is_success) {
    $validations = $c->device_report->validation_results($device_report->value->{id});
...
sub set_triton_setup ($c) {
  ...
  return $c->status(409, { error => 
    ...
    "UUID set before it can be marked as set up for Triton"
      ...
      defined( $device->latest_triton_reboot ) && defined( $device->triton_uuid )
    );
