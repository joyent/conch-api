package Mojo::Conch::Controller::WorkspaceRoom;
...
sub replace_rooms ($c) {
  ...
  unless ( $body && ref($body) eq 'ARRAY' ) {
    return $c->status(400, { error => 'Array of datacenter room IDs required in request' });
  }
  ...
  my $room_attempt = $c->workspace_room->replace_workspace_rooms($workspace->id, $body);
