...
around 'Conch::Plugin::Rollbar::ROLLBAR_ENDPOINT' => sub { '/api/1/item' };
...
my $exception_payload = {
    ...
    data => {
        ...
        (map +($_ => ignore), qw(timestamp code_version platform fingerprint uuid)),
        language => 'perl',
        ...
        server => subhashof({ map +($_ => ignore), qw(cpu host root branch code_version) }),
        custom => {
            request_id => code(sub { $_[0] eq $request_id }),
            stash => all(
        ...
        # optional, that we don't send: level framework context client title notifier
    },
};
